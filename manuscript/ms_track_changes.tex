...
%DIF DEL old_ms.tex       Wed May 23 23:02:27 2018
%DIF ADD manuscript.tex   Wed May 23 14:31:15 2018
...
MH proposal ratio in \DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite[p. 287]{yang2014molecular}}\hspace{0pt}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citet[p. 287]{yang2014molecular}}\hspace{0pt}%DIFAUXCMD
...
multiplier to subclades of the tree. MH proposal ratios for branch length updates follow the derivations given for the  the 'multiplier' or 'proportional scaling'
 move described by \DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite[p. 225]{yang2014molecular}}\hspace{0pt}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citet[p. 225]{yang2014molecular}}\hspace{0pt}%DIFAUXCMD
}\DIFaddend .
...
datasets, which are \DIFdelbegin \DIFdel{typically }\DIFdelend \DIFaddbegin \DIFadd{often }\DIFaddend compiled from only one or two \DIFdelbegin \DIFdel{organs.}\DIFdelend \DIFaddbegin \DIFadd{anatomical regions (eg., "cranium", "pelvis", "leaf"). 
}\DIFaddend In the future, \DIFdelbegin \DIFdel{I am hopeful that }\DIFdelend \DIFaddbegin \DIFadd{methods such as that described here may encourage the assembly of more comprehensive }\DIFaddend quantitative morphometric datasets\DIFdelbegin \DIFdel{will
increase in size to encompass much broader sampling across organs}\DIFdelend , but
at present, it seemed most sensible to examine the level of sampling
expected from existing datasets. \DIFdelbegin \DIFdel{Unlike in a previous paper
(Parins-Fukuchi 2017), I also did not simulate traits that display
covariance among sites}\DIFdelend \DIFaddbegin \DIFadd{Each simulated trait was evolved independently (ie. displaying no covariance with other sites)}\DIFaddend . 
This is because 1) I showed in \DIFdelbegin \DIFdel{the previous study that }\DIFdelend \DIFaddbegin \DIFadd{a previous study \mbox{%DIFAUXCMD
\citep{parins2017use}
}\hspace{0pt}%DIFAUXCMD
that sitewise }\DIFaddend covariance does not in and of itself significantly handicap reconstructions from
...
reconstructed insertion branch. \DIFaddbegin \DIFadd{These distances were divided by the largest possible 
distance between two tips in the simulated tree to yield a measure of placement error falling between 0 and 1.
 }\DIFaddend Placement accuracy was evaluated using the
...
\DIFdelbegin \DIFdel{with the reconstructed position of each fossil placed less than 0.1
nodes away from the true position }\DIFdelend \DIFaddbegin \DIFadd{displaying 0.3\% error  }\DIFaddend on average. In the presence of random
...
tend to fall within the correct region of the tree, \DIFdelbegin \DIFdel{falling 1.85 and \textasciitilde{}3 nodes away from the correct placement }\DIFdelend \DIFaddbegin \DIFadd{exhibiting between 7.8\% (with binary weighting under the compound Dirichlet prior) and 17.6\% (unweighted, under the exponential prior)  
placement error }\DIFaddend on average under the exponential prior when
alignments contain an equal number of clean and dirty sites. And although the procedure
reconstructs fossil positions that are quite distant in the worst case (\DIFdelbegin \DIFdel{3 nodes away, on average}\DIFdelend \DIFaddbegin \DIFadd{18\% error under the exponential prior with no weighting scheme}\DIFaddend ),
application of the weighting procedures \DIFdelbegin \DIFdel{improves reconstructions}\DIFdelend \DIFaddbegin \DIFadd{reduces placement error by half}\DIFaddend , even though
...
\DIFdelbegin \DIFdel{The mean distance between the true positions of fossils and their inferred placements is
smaller }\DIFdelend \DIFaddbegin \DIFadd{Placement error  lower }\DIFaddend under the compound Dirichlet in all but one of the \DIFdelbegin \DIFdel{comparisions}\DIFdelend \DIFaddbegin \DIFadd{comparisons}\DIFaddend . The improvement
exhibited under the compound Dirichlet is greatest when using the binary weighting scheme,
resulting in \DIFdelbegin \DIFdel{placements approximately 0.2-0.4 nodes closer to the true position across all
simulated }\DIFdelend \DIFaddbegin \DIFadd{a  2\% reduction in error compared to exponential prior on the noisiest }\DIFaddend datasets.
...
beneficial in cases where the distribution of noise varies \DIFdelbegin \DIFdel{betwen
}\DIFdelend \DIFaddbegin \DIFadd{between
}\DIFaddend different regions of trees. This is because the float weighting scheme
...
\textbf{Table 1.} Mean \DIFdelbegin \DIFdel{distances of true and reconstructed fossil
placements }\DIFdelend \DIFaddbegin \DIFadd{error when placing simulated fossils }\DIFaddend under the exponential branch length prior. \DIFdelbegin \DIFdel{Distances are }\DIFdelend \DIFaddbegin \DIFadd{Error is }\DIFaddend measured as the average number of nodes
separating reconstructed placements from their true positions across all
100 replicates of each dataset \DIFaddbegin \DIFadd{divided by the maximum possible path length between nodes}\DIFaddend .
...
\midrule
\endhead
50 clean & \DIFdelbegin \DIFdel{0.054  }\DIFdelend \DIFaddbegin \DIFadd{0.003  }\DIFaddend & \DIFdelbegin \DIFdel{0.053 }\DIFdelend \DIFaddbegin \DIFadd{0.003 }\DIFaddend & \DIFdelbegin \DIFdel{0.072 }\DIFdelend \DIFaddbegin \DIFadd{0.004 }\DIFaddend \tabularnewline
50 clean + 10 dirty & \DIFdelbegin \DIFdel{0.131 }\DIFdelend \DIFaddbegin \DIFadd{0.007 }\DIFaddend & \DIFdelbegin \DIFdel{1.192 }\DIFdelend \DIFaddbegin \DIFadd{0.063 }\DIFaddend & \DIFdelbegin \DIFdel{2.466 }\DIFdelend \DIFaddbegin \DIFadd{0.130 }\DIFaddend \tabularnewline
50 clean + 25 dirty & \DIFdelbegin \DIFdel{0.751 }\DIFdelend \DIFaddbegin \DIFadd{0.040 }\DIFaddend & \DIFdelbegin \DIFdel{2.552 }\DIFdelend \DIFaddbegin \DIFadd{0.134 }\DIFaddend & \DIFdelbegin \DIFdel{3.175 }\DIFdelend \DIFaddbegin \DIFadd{0.167 }\DIFaddend \tabularnewline
50 clean + 50 dirty & \DIFdelbegin \DIFdel{1.474 }\DIFdelend \DIFaddbegin \DIFadd{0.078 }\DIFaddend & \DIFdelbegin \DIFdel{2.982  }\DIFdelend \DIFaddbegin \DIFadd{0.156  }\DIFaddend & \DIFdelbegin \DIFdel{3.324}\DIFdelend \DIFaddbegin \DIFadd{0.175 }\DIFaddend \tabularnewline
\bottomrule
\end{longtable}

\textbf{Table 2.} Mean \DIFdelbegin \DIFdel{distances of true and reconstructed fossil
placements }\DIFdelend \DIFaddbegin \DIFadd{error when placing simulated fossils }\DIFaddend under the compound Dirichlet branch length prior. \DIFdelbegin \DIFdel{Distances are }\DIFdelend \DIFaddbegin \DIFadd{Error is }\DIFaddend measured as the average number of nodes
separating reconstructed placements from their true positions across all
100 replicates of each dataset \DIFaddbegin \DIFadd{divided by the maximum possible path length between nodes}\DIFaddend .
